...
@Component({
  selector: 'app-assignee-ticket-dialog',
  templateUrl: './assignee-ticket-dialog.component.html',
  styleUrls: ['./assignee-ticket-dialog.component.css']
})
export class AssigneeTicketDialogComponent implements OnInit {
  currentTicketId = 0;
  ...
  constructor(private ticketService: TicketService, private dictionaryService: DictionaryService,
              public dialogRef: MatDialogRef<AssigneeTicketDialogComponent>,
  ...
  assigneeTicket(ticketId: any): void {
    this.ticketService.changeStatusTicket(ticketId, 'COMPLETE').subscribe(
      res => {
        this.dialogRef.close();
      });
  }
